    ...
    >
      <Text
        className={`text-sm font-semibold ${
          expired ? "text-red-400" : urgent ? "text-amber-400" : "text-emerald-400"
        }`}
      >
        {expired
    ...
    >
      ...
      {loading ? (
        ...
        <>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View className="flex-row gap-2">
                  {vehicles.map((v, i) => (
                    ...
                    >
                      <Text
                        className={`font-bold tracking-wider ${i === selectedIdx ? "text-emerald-300" : "text-white"}`}
                      >
                        {v.plateNumber}
                      </Text>
                    </Pressable>
          ...
          {/* QR Card */}
          <View className="bg-gray-900 rounded-3xl p-6 items-center border border-gray-800">
            {/* Plate number header */}
            ...
            {selected?.status !== "approved" ? (
              ...
              <View className="items-center w-full">
                ...
                <Pressable
                  ...
                  disabled={issuing}
                  className="mt-4 py-3 px-6 rounded-xl bg-gray-800 active:bg-gray-700"
                >
                  {issuing ? (
                    <ActivityIndicator color="#10b981" />
                  ) : (
                    <Text className="text-gray-300 text-sm">↺ Renew Pass</Text>
                  )}
                </Pressable>
              </View>
            ) : (
              <View className="items-center py-8 w-full">
                ...
                <Text className="text-gray-300 font-semibold text-center mb-2">
                  No Active Pass
                </Text>
                ...
                <Pressable
                  onPress={handleIssuePass}
                  disabled={issuing}
                  className="bg-emerald-600 rounded-xl py-3.5 px-8 active:bg-emerald-700"
                >
                  {issuing ? (
                    <ActivityIndicator color="#fff" />
                  ) : (
                    <Text className="text-white font-bold">Generate QR Pass</Text>
                  )}
                </Pressable>
              </View>
            )}
          ...
          {/* Info */}
          <View className="bg-gray-900/60 rounded-2xl p-4 mt-4 border border-gray-800">

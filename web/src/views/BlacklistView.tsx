...
export const BlacklistView: React.FC = () => {
  ...
  const fetchBlacklist = async (showRefreshing = false) => {
    ...
    try {
      ...
      setActionMessage({
        type: "error",
        text: err.message || "Failed to load blacklist",
      });
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleRemoveFromBlacklist = async (item: VehicleItem) => {
    ...
    try {
      ...
      setActionMessage({
        type: "error",
        text: err.message || "Failed to remove from blacklist",
      });
    } finally {
  ...
  const handleAddBlacklist = async (e: React.FormEvent) => {
    ...
    try {
      await api.setBlacklistStatus(
        targetVehicleId.trim(),
        true,
        blacklistReason.trim() || "Security restriction",
      );
      setActionMessage({
        type: "success",
      ...
      setActionMessage({
        type: "error",
        text: err.message || "Failed to blacklist vehicle",
      });
    } finally {
      setProcessingId(null);
    }
    ...
    >
      ...
      >
        <div>
          ...
          >
            Vehicles listed here trigger an immediate red alarm and automated
            barrier lockdown at gate checkpoints.
          </p>
        </div>
                ...
                <tr>
                  ...
                  >
                    {loading
                      ...
                      : "No vehicles currently blacklisted. Campus gate security clear."}
                  </td>
                </tr>
              ) : (
                    ...
                    <tr key={item.id}>
                      ...
                      <td style={{ color: "#fca5a5", fontSize: "0.85rem" }}>
                        {item.blacklistReason ||
                          "Security restriction flagged by supervisor"}
                      </td>
                      <td>
                        <button

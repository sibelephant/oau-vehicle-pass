...
import { api, type TrafficSummary, type PeakHourItem, type AccessLogItem } from "../lib/api";
...
export const OverviewView: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [sumRes, peakRes, logRes] = await Promise.all([
        ...
        api.getAccessLog({ limit: 6 }).catch(() => ({
          data: [] as AccessLogItem[],
          page: 1,
          limit: 6
        })),
      ]);
  ...
  const approvalRate = summary.totalEntries > 0
    ...
    <div style={{ padding: "32px", display: "flex", flexDirection: "column", gap: "28px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginTop: "4px" }}>
            Real-time traffic flow, channel breakdown, and security alerts for OAU Main Gate.
          </p>
        </div>
      ...
      <div style={{
        ...
        <div className="glass-panel" style={{ padding: "20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "6px", marginTop: "14px", fontSize: "0.8rem", color: "var(--text-muted)" }}>
            <span style={{ color: "#34d399", fontWeight: 700, display: "flex", alignItems: "center" }}>
              <TrendingUp size={14} style={{ marginRight: "2px" }} /> {approvalRate}%
            </span>
        ...
        <div className="glass-panel" style={{ padding: "20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <div style={{ fontSize: "2rem", fontWeight: 800, marginTop: "8px", color: "#fbbf24" }}>
      ...
      <div style={{
        ...
        <div className="glass-panel" style={{ padding: "24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.825rem", marginBottom: "6px" }}>
                ...
                <span className="mono-text" style={{ color: "var(--text-muted)" }}>
                  {summary.qrCount} ({summary.totalEntries > 0 ? Math.round((summary.qrCount / summary.totalEntries) * 100) : 0}%)
                </span>
              </div>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.825rem", marginBottom: "6px" }}>
                ...
                <span className="mono-text" style={{ color: "var(--text-muted)" }}>
                  {summary.anprCount} ({summary.totalEntries > 0 ? Math.round((summary.anprCount / summary.totalEntries) * 100) : 0}%)
                </span>
              </div>
              <div style={{ height: "8px", background: "rgba(255, 255, 255, 0.08)", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.825rem", marginBottom: "6px" }}>
                ...
                <span className="mono-text" style={{ color: "var(--text-muted)" }}>
                  {summary.manualCount} ({summary.totalEntries > 0 ? Math.round((summary.manualCount / summary.totalEntries) * 100) : 0}%)
                </span>
              </div>
              <div style={{ height: "8px", background: "rgba(255, 255, 255, 0.08)", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{
          ...
          <div style={{
            ...
            💡 <strong>System Note:</strong> The study’s dual-channel approach ensures that when plate reading is impeded by rain or headlight glare, the QR pass acts as an instantaneous fallback.
          </div>
        </div>
          ...
          <div style={{
            ...
            {peakHours.map((item) => {
              ...
              return (
                ...
                >
                  <span style={{ fontSize: "0.65rem", color: isPeak ? "#fbbf24" : "var(--text-dim)", fontWeight: isPeak ? 700 : 500 }}>
                    {item.count}
                  </span>
                  <div
                    style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
          <div>
            ...
            <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginTop: "2px" }}>
              Most recent verifications and flag alerts recorded at the campus barrier
            </p>
          </div>
        </div>
              ...
              {recentLogs.length === 0 ? (
                ...
                recentLogs.map((item) => (
                  <tr key={item.log.id}>
                    ...
                    <td>
                      <span style={{ textTransform: "capitalize", fontSize: "0.85rem", color: "var(--text-main)" }}>
                    ...
                    <td>
                      <span className={`badge badge-${item.log.decision === "granted" ? "success" : "danger"}`}>
                        {item.log.decision}
                      </span>
                    </td>
